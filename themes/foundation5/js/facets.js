...
function buildFacetNodes(data, currentPath, allowExclude, excludeTitle, counts)
{
  ...
  $(data).each(function facetNodesEach() {
    ...
    if (!this.isApplied && counts) {
      html = '<span class="badge right">' + this.count.toString().replace(/\B(?=(\d{3})+\b)/g, VuFind.translate('number_thousands_separator'));
      if (allowExclude) {
    ...
    if (this.operator === 'OR') {
      ...
      html += '<i class="fa fa-check right" title="' + VuFind.translate('Selected') + '"></i>';
    }
